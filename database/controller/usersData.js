...
module.exports = async function (databaseType, userModel, api, fakeGraphql) {
	let Users = [];
	const cfg = (global.GoatBot && global.GoatBot.config && global.GoatBot.config.database) || {};
	const dataDir = cfg.jsonDataDir || process.env.JSON_DATA_DIR || path.join(__dirname, "..", "data");
	const pathUsersData = path.join(dataDir, "usersData.json");
	...
	async function create_(userID, userInfo) {
		...
		const queue = new Promise(async function (resolve_, reject_) {
			try {
				...
				if (!userInfo) {
					try {
						const info = api?.getUserInfo ? await api.getUserInfo(userID) : null;
				...
				let userData = {
					...
					gender: (userInfo.gender !== undefined && userInfo.gender !== null) ? userInfo.gender : null,
		...
		return new Promise((resolve, reject) => {
			taskQueue.push(async function () {
				try {
					...
					if (!global.db.allUserData.some(u => u.userID == userID))
						await create_(userID);
					const currentMoney = Number(await get_(userID, "money")) || 0;
					const newMoney = currentMoney + Number(money);
