...
module.exports = {
  ...
  onStart: async function ({ api, event }) {
    try {
      ...
      const outputPath = path.join(cacheDir, `c_panel_${Date.now()}.gif`);
      ...
      for (let i = 0; i < 5; i++) {
        ...
        ctx.fillText("SIDDIK BOT V2", 30, 40);
      ...
      if (typeof botInstance.sendAnimation === "function") {
        await botInstance.sendAnimation(threadID, fs.createReadStream(outputPath), {}, { reply_to_message_id: messageID });
      } else if (typeof botInstance.sendDocument === "function") {
        await botInstance.sendDocument(threadID, fs.createReadStream(outputPath), {}, { reply_to_message_id: messageID });
      } else {
      ...
      const threadID = event.threadID || event.chat?.id;
      const messageID = event.messageID || event.message_id;
      api.sendMessage("❌ Failed to generate panel.", threadID, () => {}, messageID);
    }
  }
};
