...
module.exports = {
	...
	onStart: async function ({ args, message, envGlobal, getLang }) {
		...
		const cacheDirW = pathW.join(__dirname, "cache");
		if (!fsW.existsSync(cacheDirW)) fsW.mkdirSync(cacheDirW, { recursive: true });
		const tmpPathW = pathW.join(cacheDirW, `weather_${Date.now()}.jpg`);
		fsW.writeFileSync(tmpPathW, imageBuffer);
		const result = await message.reply({
			body: msg,
			attachment: fsW.createReadStream(tmpPathW)
		});
		setTimeout(() => { try { fsW.unlinkSync(tmpPathW); } catch (e) {} }, 12000);
		return result;
	}
};
